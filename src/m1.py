...
class CircleChanger(object):
    ...
    def get_distance_from(self, point):
        ...
        ################################################################
        return math.sqrt((point.x - self.circle.center.x) ** 2 + (point.y -
                                                             self.circle.center.y) ** 2)
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        ################################################################
        new_radius = self.circle.radius + amount_to_swell_or_shrink
        if new_radius >= 1:
    ...
    def swallow(self, other_circle_changer):
        ...
        center = rg.Point(x, y)
        radius = self.get_distance_from(center)
        new_colors = self.colors + other_circle_changer.colors

        new_cc = CircleChanger(x, y, radius, 'red', new_colors)
    ...
    def change_to_next_color_in_tuple(self):
        ...
        ################################################################
        if self.has_run_next_color:
            new_index = self.index_of_current_color + 1
            if new_index > len(self.colors) - 1:
                new_index = 0

            self.index_of_current_color = new_index
            self.circle.fill_color = self.colors[new_index]
        else:
            self.index_of_current_color = 0
            self.circle.fill_color = self.colors[0]

        self.has_run_next_color = True
